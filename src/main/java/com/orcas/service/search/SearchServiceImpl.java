...
import com.orcas.repository.HouseDetailRepository;
import com.orcas.repository.HouseRepository;
import com.orcas.repository.HouseTagRepository;
...
import com.orcas.service.search.HouseIndexKey;
import com.orcas.service.search.HouseIndexTemplate;
...
public class SearchServiceImpl implements ISearchService {
    ...
    private void createOrUpdateIndex(HouseIndexMessage message) {
        ...
        modelMapper.map(house, indexTemplate);  //把house映射成indexTemplate
    ...
    private void removeIndex(HouseIndexMessage message) {
        ...
        if (!serviceResult.isSuccess() || deleted <= 0) {
            logger.warn("Did not remove data from es and lbs for response: {}, and for serviceResult: {} " + response, serviceResult);
            // 重新加入消息队列
    ...
    private boolean deleteAndCreate(long totalHit, HouseIndexTemplate indexTemplate) {
        ...
        long deleted = response.getDeleted();
        if (deleted != totalHit) {
            logger.warn("Need to delte {}, but {} was deleted!", totalHit, deleted);
            return false;
        } else {
